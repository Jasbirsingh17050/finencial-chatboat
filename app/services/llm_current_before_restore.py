...
def calculate_dcf(
    ...
    try:
        ...
        return (
            ...
            f"- **Starting Free Cash Flow (FCF):** "
            f"${initial_fcf:,.2f}M\n"
            f"- **Projected Growth Rate ({years} Yrs):** "
            f"{growth_rate}%\n"
            f"- **Discount Rate (WACC):** "
            f"{discount_rate}%\n"
            f"- **Terminal Growth Rate:** "
            f"{terminal_growth}%\n"
            f"- **Sum of PV Cash Flows:** "
            f"${sum(pv_cf):,.2f}M\n"
            f"- **PV of Terminal Value:** "
            f"${pv_terminal_value:,.2f}M\n"
            f"- **Estimated Enterprise Intrinsic Value:** "
            f"**${enterprise_value:,.2f} Million**\n"
        )
...
def calculate_emi(
    ...
    return (
        ...
        f"- **Monthly EMI Payment:** "
        f"**${emi:,.2f}**\n"
        f"- **Total Interest Payable:** "
        f"${total_interest:,.2f}\n"
...
def parse_agentic_math(
    ...
    if (
        "dcf" in q
        or "discounted cash flow" in q
        or "intrinsic value" in q
    ):
...
def fetch_live_stock_data(
    ...
    for key, ticker in global_overrides.items():
        ...
        if (
            key in query_lower
            and ticker not in potential_tickers
        ):
    ...
    if not potential_tickers:
        ...
        if combined_search:
            ...
            try:
                ...
                print(
                    f"Yahoo symbol search failed: {e}"
                )
    ...
    for ticker in potential_tickers[:5]:
        ...
        try:
            ...
            market_data += (
                f"Live/Latest Available Price: "
                f"{currency} {price:.2f}\n"
            )

            market_data += (
                f"Price Approximately 1 Year Ago: "
                f"{currency} {price_1y_ago}\n"
            )

            market_data += (
                f"Market Cap: {market_cap_str} "
                f"(Raw Digits in Billions: "
                f"{market_cap_b}) | "
            ...
            market_data += (
                f"Current Month Average Price "
                f"(Last 30 Days): "
                f"{avg_last_30}\n"
            )

            market_data += (
                f"Previous Month Average Price "
                f"(30 Days Prior): "
                f"{avg_prev_30}\n"
            )

            market_data += (
                f"Difference in Averages "
                f"(Current - Previous): "
                f"{diff_avg}\n"
            )

            market_data += (
                f"Current Month Median Price: "
                f"{median_last_30}\n"
            )

            market_data += (
                f"Recent 7-Day History: "
                f"{recent_prices}\n\n"
            )
...
def stream_financial_response(
    ...
    if (
        ...
        role = user_profile.get(
            "professional_role",
            "Unknown",
        )

        focus = user_profile.get(
            "focus_area",
            "Unknown",
        )
    ...
    system_prompt = f"""
You are FinanceVision AI, an elite financial
advisory engine and corporate RAG assistant.
...
You are strictly a financial and corporate AI.
Refuse non-financial topics like cooking or sports.

2. CITATION MANDATE:
When answering questions using data from the
[UPLOADED COMPANY KNOWLEDGE BASE],
explicitly cite the page and document source.
...
use the exact values supplied by the application.

Do not invent live prices.

Do not replace a supplied live price with
an estimated value.

If live market data is unavailable,
clearly say that the latest Yahoo Finance
data could not be retrieved.

4. FINANCIAL EXPERTISE:
Answer stock analysis, sector queries,
company comparisons, valuations,
financial concepts and market questions.

5. MANDATORY COMPARISON BAR GRAPHS:

If the user asks for PRICE COMPARISON
across time:

[COMPARE_CHART|Stock Price Comparison (in Currency)|1 Year Ago:RawNumber1|Today:RawNumber2]

If the user asks to compare companies,
market caps, revenues or net worth:

[COMPARE_CHART|Chart Title (Unit)|Label1:RawNumber1|Label2:RawNumber2|Label3:RawNumber3]

Numbers must contain raw digits only.
...
6. TIME-SERIES LINE CHARTS:

If the user asks for a specific stock
price trend or line chart:

[INTERACTIVE_CHART|TICKER_SYMBOL|line]

7. INVESTMENT ADVISOR FRAMEWORK:
...
def get_global_indices():
    ...
    # --------------------------------------------------------
    # No hard-coded fake market data.
    # --------------------------------------------------------
